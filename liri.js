...
function menu() {
    const commands = ["my-tweets", "spotify-this-song", "movie-this", "do-what-it-says", "exit"];
    let chosenCommand = "";
    const inquiry = [{
        type: "list",
...
function spotifyThisSong(args) {
    ...
    spotify.search({ type: 'track', query: songName, limit: 1 }, function(err, data) {
        ...
        displayResults(trackInfo, "Spotify results");
        return menu();
    });
}
...
function movieThis(args) {
    ...
    request(queryUrl, function(error, response, body) {
        ...
        request(url, function(error, response, body) {
            ...
            displayResults(movieInfo, "OMDB Results");
            return menu();
